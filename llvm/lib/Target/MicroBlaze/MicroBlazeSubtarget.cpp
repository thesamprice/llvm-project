...
// listed for the generic ProcessorModel in MicroBlaze.td (e.g. +pattern-compare)
// are applied.
...
void MicroBlazeSubtarget::initLibcallLoweringInfo(
  ...
  const struct { RTLIB::Libcall Op; RTLIB::LibcallImpl Impl; } Calls[] = {
    ...
    // i64 arithmetic: LLVM expands all 64-bit ops via libcalls on this 32-bit target.
    ...
    // Memory intrinsics: getMemcpy/getMemset/getMemmove use SelectionDAG::Libcalls
    // (the analysis-pass copy), not TargetLowering::Libcalls. Both copies are
    // populated via initLibcallLoweringInfo, so registrations here are required.
