...
//   -mxl-soft-float         -hard-float  (C_USE_FPU=0; software emulation,
//   default)
...
void microblaze::getMicroBlazeTargetFeatures(const Driver &D,
                                             const ArgList &Args,
  ...
  if (Arg *A = Args.getLastArg(options::OPT_mxl_pattern_compare,
    ...
    Features.push_back(A->getOption().matches(options::OPT_mxl_pattern_compare)
                           ? "+pattern-compare"
                           : "-pattern-compare");
  ...
  if (Arg *A = Args.getLastArg(options::OPT_mno_xl_soft_div,
    ...
    Features.push_back(A->getOption().matches(options::OPT_mno_xl_soft_div)
                           ? "+divide"
                           : "-divide");

  if (Arg *A = Args.getLastArg(options::OPT_mxl_multiply_high,
                               options::OPT_mno_xl_multiply_high))
    Features.push_back(A->getOption().matches(options::OPT_mxl_multiply_high)
                           ? "+multiply-high"
                           : "-multiply-high");
  ...
  if (Arg *A = Args.getLastArg(options::OPT_mno_xl_soft_float,
    ...
    Features.push_back(A->getOption().matches(options::OPT_mno_xl_soft_float)
                           ? "+hard-float"
                           : "-hard-float");
  ...
  if (Arg *A = Args.getLastArg(options::OPT_mxl_float_convert,
    ...
    Features.push_back(A->getOption().matches(options::OPT_mxl_float_convert)
                           ? "+float-convert"
                           : "-float-convert");
}
